function [sparse_rep,sparse_recon,quant,nmses,quants,sparsity,q_max,bpp,energy_ratio,means] = sparsify_D3_mse(Data,nmse,ind_nnz)
...
% Take transform
dctc = mirt_dctn(Data); % forward DCT

sparse_rep = dctc;

sparse_recon = mirt_idctn(dctc) + means;
...
while (searching)
    ...
    temp_recon = mirt_idctn(temp_sp) + means;
...
disp(['The final NMSE for ', num2str(nmse),' 3D DCT is ',num2str(NMSE)])
